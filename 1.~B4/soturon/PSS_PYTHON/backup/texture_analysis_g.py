...
class TextureAnalysis():
	def __init__(self, image, xres, yres, n, k, m):
		...
		self.XRES = xres # horizontal resolution (must be odd)
		self.YRES = yres # vertical resolution (must be odd)
	...
	def analyse(self):
		...
		vari = self.LR_CA[la,la]
		## matlabに合わせる．
		if ( vari / self.IM_MAR[1] ) > 1e-6:
			self.LR_MAR[2] = np.mean(im.flatten()**3) / vari**1.5
			self.LR_MAR[3] = np.mean(im.flatten()**4) / vari**2
		else:
			self.LR_MAR[2] = 0
			self.LR_MAR[3] = 3
		...
		_b_p, _b_rp, _b_ip = sutils.get_parent_g(copy.deepcopy(_sp.BND))
		...
		self.H0_PRO = np.var(_b['s'].real,ddof=0)
		...
		for dp in range(self.N-1, -1, -1):
			...
			for k in range(self.K):
 			#	_recon += _s.B_FILT[0][k] * bnd[dp][k]['f']
				_recon += ( (1j)**(self.K-1) ) * _s.B_FILT[0][k] * np.fft.fftshift( np.fft.fft2( bnd[dp][k]['s'].real ) )
			#	_recon += ( (1j)**(self.K-1) ) * _s.B_FILT[0][k] * bnd[dp][k]['f']
